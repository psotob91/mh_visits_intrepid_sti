...
- When we fit harmonic(month, 2, 12), we get 4 regressors: 

$$cos(\frac{2\times\pi\times\text{month}}{12})$$

$$sin(\frac{2\times\pi\times\text{month}}{12})$$

$$cos(\frac{4\times\pi\times\text{month}}{12})$$

$$sin(\frac{4\times\pi\times\text{month}}{12})$$
- When we fit harmonic(month, 1, 12), we get 2 regressors: 
...
- A pre-specified and more complex model is the following:
...
```{r}
compare_parameters(mod_full$mod, mod_full$mod_harm1, mod_full$mod_noharm, select = "{estimate}{stars} ({p})") 
```
...
```{r}
parameters::model_parameters(mod_full$mod_harm1) |> 
  dplyr::select(Parameter, Coefficient, CI_low, CI_high, p) |> 
  filter(Parameter %in% c("To", "To:I(time - tpand)")) |> 
  as_data_frame() |> 
  rename(est = Coefficient, 
...
```{r}
parameters::model_parameters(mod_full$mod_noharm) |> 
  dplyr::select(Parameter, Coefficient, CI_low, CI_high, p) |> 
  filter(Parameter %in% c("To", "To:I(time - tpand)")) |> 
  as_data_frame() |> 
  rename(est = Coefficient, 
