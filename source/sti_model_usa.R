sti_model_usa <- function(country, mh_category, p , q, data, 
                             shock_exclude = TRUE) {
  ...
  if (shock_exclude == TRUE) {
    ...
    mh_visitsm <- mh_visitsm |> 
      mutate(time_adjusted = time - tpand, 
             ...
             numerator = case_when(
               USA_systdown == 0 ~ numerator, 
               USA_systdown == 1 ~ as.numeric(NA)
               ), 
             denominator = case_when(
               USA_systdown == 0 ~ denominator, 
               USA_systdown == 1 ~ as.numeric(NA)
             ),             
             ) 
    
    mh_visitsm <- mh_visitsm |> 
      mutate(numerator = na_interpolation(numerator, option = "stine"),
             numerator = round(numerator, 0), 
             denominator = na_interpolation(denominator, option = "stine"), 
